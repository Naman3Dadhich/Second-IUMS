...
const Steps = ({ step, adminData, setEdit, edit }) => {
  switch (step) {
    case 1:
      return (
        <>
          <div className="personalInformation">
            ...
            {/* <h5 className="dataBlock">
             Faculty's Name: <span> {adminData.name}</span>
           </h5>
           <h5 className="dataBlock">
             Email: <span> {adminData.email}</span>
           </h5> */}
            <h5 className="dataBlock">
              Phone No: <span>{adminData.phoneNo}</span>
            </h5>
            <h5 className="dataBlock">
              Qualification: <span>{adminData.Qualification}</span>
            </h5>
            <h5 className="dataBlock">
              Designation: <span>{adminData.Designation}</span>
            </h5>
            <h5 className="dataBlock">
              Expertise: <span> {adminData.Expertise}</span>
            </h5>
            {/* <h5 className="dataBlock">
             Pincode: <span>{adminData.pinCode}</span>
           </h5> */}
          </div>
        </>
      );
      ...
      return (
        <>
          ...
          <div className="AchievementsCardsBlock">
            ...
            {adminData ? (
              adminData.achievements.map((element) => {
                return (
      ...
      return (
        <>
          ...
          <div className="AcademicQualification">
            <h5 className="dataBlock">
              Phd.: <span>{adminData.phoneNo}</span>
            </h5>

            <h5 className="dataBlock">
              Post Graduation: <span>{adminData.phoneNo}</span>
            </h5>
            <h5 className="dataBlock">
              Graduation: <span>{adminData.phoneNo}</span>
            </h5>
          </div>
        </>
      );
...
function SuperUserProfile({ ansData }) {
  ...
  const [adminData, setfacultyData] = useState({
    name: "",
    email: "",
    phoneNo: "",
    Qualification: "",
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("/profile/superadmin/edit", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
            ...
            {editBasicDetails ? (
              ...
              <>
                <p style={{ fontSize: 1.7 + "em" }}>
                  Name: <span style={{ color: "white" }}>{adminData.name}</span>
                </p>
